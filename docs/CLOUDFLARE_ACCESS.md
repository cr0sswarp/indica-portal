...
`portal.indicalab.jp` を **Cloudflare Access (Zero Trust)** で保護し、
...
1. https://dash.cloudflare.com → **Add a site** → `indicalab.jp` を入力。
2. プラン選択 (Free で可)。
3. Cloudflare が表示する2つのネームサーバを、ドメインレジストラ
   (お名前.com / Route53 等) の NS 設定に登録。
4. 反映まで数時間〜最大48h。`dig NS indicalab.jp +short` で確認。
...
- **Application domain**: `portal.indicalab.jp` (パス無し = サイト全体)
- Next。
...
1. シークレットウィンドウで `https://portal.indicalab.jp` を開く。
...
現状 `index.html` は HTMLタグを含まないプレーンテキストになっている
(`grep -c '<html' index.html` → 0)。Access は中身に関わらずドメインを
保護できるが、**ポータルページ自体は実HTMLとして作り直す必要がある**。
`skills/valiente-portal-v3.skill` のテンプレートを基に再生成するのが早い。
