...
MAX_TRACK_DIST = 120    # ピクセル距離上限 (旧80→120: 速い動きに対応)
MAX_DISAPP   = 60       # 消失許容フレーム数 (旧30→60: 2秒間の遮蔽に耐える)
ANCHOR_WINDOW  = 0.18   # minutes: ±10.8秒がアンカーとしてマッチ
ANCHOR_MAX_DIST = 18.0
ANCHOR_WARP_LIMIT = 20.0  # アンカー採用の最大ジャンプ距離(m): これを超えたら棄却
RECOVER_DIST = 5.0      # ID消失後の自動回復の最大距離(m)
...
# manual_tags.json があれば jersey=6 のタグをアンカーとして追加
_manual_tags_path = OUTPUT_DIR / "manual_tags.json"
if _manual_tags_path.exists():
    _mt = json.loads(_manual_tags_path.read_text())
    _added = 0
    for v in _mt.values():
        if v.get("jersey") == 6:
            anchors.append((v["time_min"], v["fx"], v["fy"]))
            _added += 1
    if _added:
        print(f"manual_tags.json から #6 アンカー {_added} 件追加")

anchors.sort(key=lambda x: x[0])

# 最初のアンカーを t=0 にも複製 → 試合開始直後から確認できる
if anchors:
    anchors.insert(0, (0.0, anchors[0][1], anchors[0][2]))

print(f"有効アンカー数: {len(anchors)} (先頭: t={anchors[0][0]:.2f}min  fx={anchors[0][1]}  fy={anchors[0][2]})")
...
total_written=0; anchor_hits=0; warp_rejected=0; t_start=time.time()
...
for half_idx,(video_path,t_offset,half_label) in enumerate([
    ...
    while True:
        ...
        if not is_anchor and jersey6_id is not None:
            ...
            if player6_det is None:
                # ── ID消失後の自動回復: 直前位置の近くにいるトラックに再接続 ──
                if jersey6_pos is not None:
        ...
        cv2.putText(out,f"Anchors: {anchor_hits}  WarpRej: {warp_rejected}",(10,40),FONT,0.5,(180,255,180),1,cv2.LINE_AA)
        if is_anchor:
        ...
        if fn%2700==0:
            e=time.time()-t_start
            print(f"  [{fn/total_f*100:.0f}%] {local_t:.0f}min  frames:{total_written}  {total_written/e:.1f}fps  {e:.0f}s  warpRej:{warp_rejected}")
