...
### 実行チェックリスト（牧野さん側）
- [x] **Step 0**: 入金 → ⚠️ **$5の予定が誤って$105入金**（カード払い）
- [ ] **Step 0.5【返金】**: カード払い＝未使用分は返金可。Vast.aiチャットサポートへ$100返金依頼。
      ※ 後述のDestroyまで終えれば消費は数十セントなので、最後に**ほぼ満額**返金請求でもOK
...
- [ ] **Step 4【返金確定】**: Destroy後、残高ほぼ全額をチャットで返金請求
- [ ] **完了報告** → COOが #1 をクローズし #2 へ進める

> 💳 返金条件（確認済み）: **使用済み=返金不可／未使用カード分=返金可／暗号資産=不可**。
> 出典: docs.vast.ai/billing, vast.ai/terms
