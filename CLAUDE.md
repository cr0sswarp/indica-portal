...
### 目的
財団設立に向けた活動の一環として、**サッカー分析ツール TZI (Tactical Zone Intelligence) を完成させること**が最優先タスク。試合映像から選手の**体の向き・重心・位置・動線**を高精度に抽出し、戦術分析に使えるプラットフォームを構築する。

### 技術選定方針
- **精度 > コスト**: 無料ツール (YOLOv8n等) に縛られない。精度向上に繋がるなら有償サービス・クラウドGPUも積極採用する
- **最善のツールを選ぶ**: 世界的に最も優秀な手法を調査・採用する。技術的負債を作らない
- **妥協しない**: 「今の環境でできる範囲」ではなく「本来あるべき姿」を目指す

### TZI コア設計原則 (必ず守ること)

```
❌ 間違った設計: 定期サンプリング → 毎回OCR再特定 → IDが不安定
✅ 正しい設計:  手動初期化タグ → 連続フレーム追跡 → タグが外れない
```

1. **手動タグ初期化**: 試合開始フレームで選手をクリック → 背番号を入力 → タグ付与
2. **連続追跡 (ByteTrack/BotSORT)**: track_id が全フレームを通じて持続する
3. **タグ持続**: `track_id → jersey_number` マッピングが試合終了まで維持される
4. **OCRは検証ツール**: タグ消失・ID切り替わり検出のための補助手段。主役ではない
5. **毎フレーム分析**: 体の向き・重心・位置は全フレーム(または5fps)で更新する

### 現在の技術スタック (変更を検討すべき項目)
| 役割 | 現状 | 理想 |
|---|---|---|
| 選手検出 | YOLOv8n | YOLOv8n or YOLO11n (OK) |
| 追跡 | Hungarian (サンプルごと) | **ByteTrack** (連続・要移行) |
| 背番号認識 | easyOCR (誤読多) | PARSeq + legibility classifier |
| 体の向き | **未実装** | RTMPose-t (ONNX) |
| 重心 | **未実装** | 腰キーポイントから推定 |
| タグ付けUI | **未実装** | **クリック → 番号入力 → タグ伝搬** |
